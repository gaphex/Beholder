...
from config import EMB_DIM, LANG_MODEL
...
from dataset import (coco, cocoXYFilenames, FoxyDataStream, GloveTransformer,
    ...
    sbuXYFilenames, Word2VecTransformer)
...
class DataETL():
    ...
    @staticmethod
    def getFinalStream(X, Y, sources, sources_k, batch_size=256, embedding_dim=EMB_DIM,
        shuffle=False):
        """
        Returns
        -------
        ...
        if LANG_MODEL == 'WORD2VEC':
            w2v = Word2Vec.load(W2V_PATH)
            print 'Loading w2v vectors...'
            train_transformer = Word2VecTransformer(w2v, data_stream=train_stream, vectorizer=vect)
            train_transformer_k = Word2VecTransformer(w2v, data_stream=train_stream_k, vectorizer=vect)
            ...
            train_transformer_k = GloveTransformer(glove_version, data_stream=train_stream_k, vectorizer=vect)
        
        
        #train_transformer.produces_examples = False
        #train_transformer_k.produces_examples = False

        # Final Data Streams w/ contrastive examples
        shuffle_batch = ShuffleBatch(train_transformer_k)
        #shuffle_batch.produces_examples = False
        final_train_stream = Merge(
              (train_transformer, shuffle_batch)
            , sources + sources_k
            )
